...
const FOOD_KEYWORDS = [
  'dish', 'plate', 'meal', 'food', 'cuisine', 'dish', 'entree', 'main',
...
const CATEGORY_MAPPING = {
  // Food categories
  'starters': ['appetizer', 'starter', 'salad', 'soup', 'bruschetta', 'tapas'],
  'main_courses': ['main', 'entree', 'dish', 'plate', 'meal', 'grilled', 'roasted'],
  'desserts': ['dessert', 'cake', 'pie', 'ice cream', 'sweet', 'pastry', 'tiramisu'],
  
  // Beverage categories
  'soft_drinks': ['juice', 'soda', 'water', 'lemonade', 'iced tea', 'smoothie'],
  'hot_beverages': ['coffee', 'tea', 'espresso', 'latte', 'cappuccino', 'hot chocolate'],
  'cocktails': ['cocktail', 'martini', 'margarita', 'mojito', 'daiquiri', 'negroni'],
  'spirits': ['vodka', 'whiskey', 'rum', 'gin', 'tequila', 'brandy', 'cognac'],
  'wines': ['wine', 'champagne', 'prosecco', 'sherry', 'port', 'sangria'],
  'beers': ['beer', 'ale', 'lager', 'stout', 'ipa', 'pilsner']
};

export function classifyMenuItem(item: MenuItem): ClassifiedItem {
...
function determineFoodCategory(text: string): string {
...
function determineBeverageCategory(text: string, isAlcoholic: boolean): string {
  const textLower = text.toLowerCase();
...
export function organizeMenuItems(items: MenuItem[]): Record<string, ClassifiedItem[]> {
