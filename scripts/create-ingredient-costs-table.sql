...
CREATE INDEX IF NOT EXISTS idx_ingredient_costs_name_lang ON ingredient_costs(ingredient_name, language);
CREATE INDEX IF NOT EXISTS idx_ingredient_costs_created ON ingredient_costs(created_at);
CREATE INDEX IF NOT EXISTS idx_ingredient_costs_confidence ON ingredient_costs(confidence_score);

-- Add RLS
ALTER TABLE ingredient_costs ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Readable by all authenticated users
DROP POLICY IF EXISTS "ingredient_costs_read_all" ON ingredient_costs;
CREATE POLICY "ingredient_costs_read_all" ON ingredient_costs
  FOR SELECT
  USING (true);

-- Only service role can insert/update ingredient costs
DROP POLICY IF EXISTS "ingredient_costs_service_only" ON ingredient_costs;
CREATE POLICY "ingredient_costs_service_only" ON ingredient_costs
  FOR ALL
  USING (false);

-- Create trigger for updated_at
DROP TRIGGER IF EXISTS update_ingredient_costs_updated_at ON ingredient_costs;
